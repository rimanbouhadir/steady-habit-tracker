...
async function init() {
    try {
        ...
        showError('Unable to connect to server. Please check your internet connection and refresh.');
    }
}
...
function showError(message) {
    ...
    grid.innerHTML = 
        ...
        '<h3 style="color:var(--accent);margin-bottom:15px;">⚠️ Connection Issue</h3>' +
        '<p style="color:var(--text-muted);margin-bottom:20px;">' + message + '</p>' +
        '<button class="btn btn-complete" onclick="location.reload()">Refresh Page</button>' +
        '</div>';
}
...
async function loadHabits() {
    try {
        ...
        showError('Failed to load habits from server. Error: ' + error.message);
    }
}
...
function renderHabits() {
    ...
    if (habits.length === 0) {
        grid.innerHTML = '<p style="text-align:center;color:var(--text-muted);padding:40px;">No habits yet. Add some!</p>';
        return;
    }
    ...
    grid.innerHTML = habits.map((habit, index) => {
        ...
        const phase1Buttons = `
            ...
            <button class="btn btn-phase" onclick="switchPhase(${habit.id}, 2)">
                Switch to Phase 2 (${habit.trigger_count || 0}/14 triggers)
            </button>
        `;
...
async function logEnergy() {
    try {
        ...
        if (!response.ok) throw new Error('Failed to log energy');
        ...
        showToast('❌ Failed to log energy');
    }
}
...
async function applyCushion() {
    try {
        ...
        showToast('✅ Micro-versions logged! Your streaks are safe.');
        await loadHabits();
        document.getElementById('cushionBox').style.display = 'none';
    } catch (error) {
        console.error('Error applying cushion:', error);
        showToast('❌ Failed to log cushion');
    }
}
...
async function submitTrigger() {
    ...
    try {
        ...
        if (!response.ok) throw new Error('Failed to log trigger');
        ...
        showToast('❌ Failed to log trigger');
    }
}
...
async function loadPatterns() {
    ...
    for (const habit of habits) {
        try {
            ...
            html += `<div class="habit-card" style="margin-bottom: 15px; color: var(--danger);">Failed to load pattern for ${habit.name}</div>`;
        }
    }
    ...
    for (const habit of habits) {
        if (habit.phase === 1 || habit.trigger_count > 0) {
            try {
                ...
                if (!response.ok) throw new Error('Trigger fetch failed');
                ...
                html += `<div class="habit-card" style="margin-bottom: 15px; color: var(--danger);">Failed to load triggers for ${habit.name}</div>`;
            }
        }
    }
